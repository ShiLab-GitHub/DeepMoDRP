...
import argparse
from model.DeepMoDRP import DeepMoDRP_Net
...
def train(modeling, train_batch, val_batch, test_batch, lr, epoch_num, cuda_name, i):
    ...
    for epoch in range(epoch_num):
        ...
        if not os.path.exists('result'):
            os.makedirs('result')
        if ret[1] < best_mse:
            torch.save(model.state_dict(), 'result/model{num}.pt'.format(num=i))
            with open('result/result{num}.csv'.format(num=i), 'w') as f:
                f.write(','.join(map(str, ret_test)))#record the test result
            best_epoch = epoch + 1
            best_mse = ret[1]
            best_pearson = ret[2]
            print(' rmse improved at epoch ', best_epoch, '; best_mse:', best_mse, model_st)#print the val result
        else:
