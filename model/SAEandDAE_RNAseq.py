...
def read_cell_line_RNAseq(filename, cell_line_dict):  # load one of the features of cell line - RNAseq
    f = open(filename, 'r')
    reader = csv.reader(f)
    for i in range(1):
        reader.__next__()
    RNAseq = [list() for i in range(len(cell_line_dict))]
    for line in reader:
        if line[0] in cell_line_dict:
            RNAseq[cell_line_dict[line[0]]] = line[1:]
    return RNAseq
...
def load_data():
    ...
    RNAseq = read_cell_line_RNAseq('../data/cellline/80cellline_17737dim_RNAseq.csv', cell_line_dict)

    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1), copy=False)
    RNAseq = min_max_scaler.fit_transform(RNAseq)
    train_data = torch.FloatTensor(RNAseq)
...
def train(train_data, test_data, data_all):
    ...
    for epoch in range(EPOCH):
        ...
        if test_loss < best_loss:
            ...
            pickle.dump(res.data.numpy(), open('../data/cellline/512dim_RNAseq.pkl', 'wb'))
